...
These tasks define the structure of tables in the Shipping section of the OM database. The tables can be designed within a smaller schema – OM Shipping schema – which include only them and the user–user_roles–roles group of the Common section. To integrate the Shipping tables developed this way into the entire OM schema, one only need to add them to other tables: no references between the Shipping tables and the tables in Customers, Vendors, Products, and Orders sections are required [to maintain the data flow in the system].
