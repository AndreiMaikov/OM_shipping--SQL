...
**Remark.**&nbsp;&nbsp;For some of the tables, only part of their columns are shown in the diagram if this does not matter for the problems considered in the next sections (for example, it does affect wave planning the `users` table contains the column holding the dates the users the users were registered in the system).
